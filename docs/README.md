# Documentation

Complete documentation for TikTok Clone Microservices project.

## 🚀 Getting Started

- **[QUICKSTART.md](QUICKSTART.md)** - Quick setup and running guide
- **[DEVELOPMENT.md](DEVELOPMENT.md)** - Development workflow and commands
- **[NATIVE_DEVELOPMENT.md](NATIVE_DEVELOPMENT.md)** - Run microservices natively for debugging

## 🏗️ Architecture & Design

- **[MONOREPO_ARCHITECTURE.md](MONOREPO_ARCHITECTURE.md)** - Monorepo structure and shared dependencies
- **[KAFKA_MIGRATION.md](KAFKA_MIGRATION.md)** - RabbitMQ to Kafka migration details

## 🤖 AI-Assisted Development

- **[AI_PROMPT_GUIDE.md](AI_PROMPT_GUIDE.md)** - Patterns, conventions, and prompt templates for AI coding

## 👥 Contributing

- **[CONTRIBUTING.md](CONTRIBUTING.md)** - How to contribute to the project
- **[COMMIT_CONVENTION.md](COMMIT_CONVENTION.md)** - Git commit message standards
- **[CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md)** - Community guidelines

## 🔒 Security

- **[SECURITY.md](SECURITY.md)** - Security best practices and policies

---

[← Back to Main README](../README.md)
